...
void Common::parse(API::Message* pMessage, DM::DataObjectPtr& dmData){
    ...
    if(!pMessage->getAttributes()->isSet("TYPE"))
      pMessage->getAttributes()->setValue("TYPE",dmData->getType()->getName());

    if(!pMessage->getAttributes()->isSet("SET"))
      pMessage->getAttributes()->setValue("SET",dmData->getType()->getURI());
