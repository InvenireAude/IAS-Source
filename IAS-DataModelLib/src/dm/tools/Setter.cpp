...
/*************************************************************************/
void Setter::setRaw(DataObjectPtr& dm,const RawContent* pRawContent)const{
	IAS_TRACER;

	IAS_LOG(LogLevel::INSTANCE.isDetailedInfo(),strXPath<<"= Raw Data:"<<pRawContent->getSize()<<" bytes.");

	if(lstPath.size())
		getParent(dm,true)->setRaw(lstPath.back().pProperty,pRawContent);
	else{
		if(!dm)
			dm=pType->createDataObject(pRawContent);
		else
			dm->setRaw(pRawContent);
	}
