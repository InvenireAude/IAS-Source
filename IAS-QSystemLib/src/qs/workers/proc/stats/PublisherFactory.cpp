...
#include <org/invenireaude/qsystem/workers/stats/Memory.h>
...
stats::Ext::SpecificationPtr PublisherFactory::SpecsToDM(const String& strSpecs){
	...
	}else if(strSpecs.compare("memory") == 0){

		dmResult->getPublishersList().add(stats::DataFactory::GetInstance()->getMemoryType()->createMemory());
    dmResult->setRefresh(99999999);

	}else{

		stats::Ext::SenderPtr dmSender(stats::DataFactory::GetInstance()->getSenderType()->createSender());
		dmSender->setOutput(strSpecs);
    dmResult->getPublishersList().add(dmSender);
	}
...
Publisher* PublisherFactory::CreatePublisher(const ::org::invenireaude::qsystem::workers::stats::Publisher* dmPublisher){
	...
	}else if(stats::DataFactory::GetInstance()->getMemoryType()->equals(pType)){
		ptrResult = IAS_DFT_FACTORY<Pub::Memory>::Create();
