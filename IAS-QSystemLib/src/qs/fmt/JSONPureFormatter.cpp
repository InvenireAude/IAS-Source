...
const String& JSONPureFormatter::CStrArray(".JSONArray");
/*************************************************************************/
...
void JSONPureFormatter::read( DM::DataObjectPtr& dmData,
    ...
    if(pAttributes && pAttributes->isSet(CStrArray)){

      if(pType) {
        dmData = pType->createDataObject();
        ptrJSONHelper->loadList(istream,dmData->getList(pAttributes->getValue(CStrArray)));
      } else {
        IAS_THROW(BadUsageException("Cannot parse input data for type:"+pAttributes->getValue(CStrTypeAttr)));
      }
...
void JSONPureFormatter::write(const DM::DataObject* dmData,
  ...
  if(pAttributes && pAttributes->isSet(CStrArray)){
    ptrJSONHelper->saveList(ostream,dmData->getList(pAttributes->getValue(CStrArray)));
  } else {
