...
void ResultHandlerForIO::handleException(::org::invenireaude::qsystem::workers::Ext::ContextPtr& dmContext,
    ...
    StringStream ssData;
	  IAS_DFT_FACTORY<DM::JSON::JSONHelper>::PtrHolder ptrJSONHelper(IAS_DFT_FACTORY<DM::JSON::JSONHelper>::Create(pDataFactory));
	  ptrJSONHelper->save(ssData, e.getValue());

		IAS_LOG(IAS::QS::LogLevel::INSTANCE.isError(),"Send to error failed, execute method"
									" does not know what to do with this exception:"+e2.toString() + ssData.str());
	}
