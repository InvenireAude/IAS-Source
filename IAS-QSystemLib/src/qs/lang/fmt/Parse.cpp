...
/*************************************************************************/
Parse::Parse(const DM::Type* pType, const StringList& lstParamaters, const IAS::Lang::Interpreter::Extern::ModuleProxy* pModuleProxy){
	IAS_TRACER;
...
void Parse::executeExternal(Exe::Context *pCtx) const{
  ...
  try{
		...
		ptrFmtFactory->getFormatter(strFormat)->read(dmValue,ssData, ptrAttributes);
...
/*************************************************************************/
Extern::Statement* Parse::Create(const DM::Type* pType, const StringList& lstParamaters, const IAS::Lang::Interpreter::Extern::ModuleProxy* pModuleProxy){
	IAS_TRACER;
	return IAS_DFT_FACTORY<Parse>::Create(pType, lstParamaters, pModuleProxy);
}
/*************************************************************************/
}
}
