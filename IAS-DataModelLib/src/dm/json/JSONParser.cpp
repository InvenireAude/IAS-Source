...
/*************************************************************************/
void JSONParser::parseList(DM::DataObjectList& lstDataObject){

  ValuesTable *pTable = new (maTables.allocate(1)) ValuesTable(maNode);
	hmRoot["Root"] = pTable;
  buildValuesTable(pTable);

  for(ValuesTable::const_iterator it=pTable->begin(); it != pTable->end(); it++)
		lstDataObject.add(buildObject(*it, lstDataObject.getType()));
